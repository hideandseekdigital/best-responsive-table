class resTable {
  ...
  onNavClick(direction) {
    ...
    if (direction == 'next') {
      ...
      diff =
        this.rows[currentIndex].getBoundingClientRect().width * currentIndex
  ...
  setCellHeightForOtherRows(cellIndex, height) {
    for (var i = 0; i < this.rows.length; i++) {
      var tr = this.rows[i]
  ...
  setCellHeightForLabels(cellIndex, height) {
